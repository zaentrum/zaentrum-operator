...
# Traefik ingress on :80, let k3s auto-apply the Zaentrum manifests, and wait
...
# The bundled Ingress now carries a REAL default host (zaentrum.localhost). Modern
# browsers auto-resolve *.localhost to 127.0.0.1, so `docker run -p 80:80 …` +
# http://zaentrum.localhost reaches Traefik with Host: zaentrum.localhost and Traefik
# matches the rule — no /etc/hosts edit, no host-strip. Crucially this keeps the
# ingress host EQUAL to the issuer host (zaentrum.localhost), the same invariant a
...
# exactly as long as k3s (the `wait` below). Signal = `get ns zaentrum` (true once
